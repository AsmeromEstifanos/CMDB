...
import { History, User, Calendar, Activity, Key } from "lucide-react";
import { formatDisplayDate } from "../utils/helpers";

const LicenseHistory = ({ licenseId, licenseName }) => {
  const { getLicenseHistory } = useAssets();
  
  const history = getLicenseHistory(licenseId);

  const getActionIcon = (action) => {
    if (action.includes("created")) return "🆕";
    if (action.includes("updated")) return "✏️";
    if (action.includes("deleted")) return "🗑️";
    if (action.includes("renewed")) return "🔄";
    if (action.includes("expired")) return "⏰";
    if (action.includes("assigned")) return "👤";
    if (action.includes("cost")) return "💰";
  ...
  const getActionColor = (action) => {
    ...
    if (action.includes("renewed")) return "bg-emerald-100 text-emerald-800";
    if (action.includes("expired")) return "bg-amber-100 text-amber-800";
    if (action.includes("assigned")) return "bg-purple-100 text-purple-800";
    if (action.includes("cost")) return "bg-indigo-100 text-indigo-800";
    if (action.includes("quantity")) return "bg-cyan-100 text-cyan-800";
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <History size={18} />
          <h3 className="text-lg font-semibold text-slate-800">License History</h3>
        </div>
        <div className="text-center py-8 text-slate-500">
          <Key size={48} className="mx-auto mb-4 text-slate-300" />
          <p className="text-lg font-medium">No history available</p>
          <p className="text-sm">This license hasn't been modified yet.</p>
        </div>
      </div>
    );
  }
